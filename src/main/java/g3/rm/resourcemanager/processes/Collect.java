...
public class Collect {
    ...
    public CompletableFuture<String> start() {
        ...
        responseService.setCollectInitEvent(task.getTaskId(), task.getSessionId());
        ...
        if (programParam == null) {
            ...
            responseService.setCollectDoneEvent(task.getTaskId(), task.getSessionId(), -1);
        ...
        responseService.setCollectDoneEvent(task.getTaskId(), task.getSessionId(), exitCode);
        if (exitCode != 0) {
            timerCreatorService.cancelTimer(timer);
            eventPublisher.publishTaskEvent(ERROR, task);
            return CompletableFuture.completedFuture(ERROR);
