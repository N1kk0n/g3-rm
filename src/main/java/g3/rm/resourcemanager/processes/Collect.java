...
public class Collect {
    ...
    public CompletableFuture<String> start() {
        ...
        Timer timer = timerCreatorService.createCollectTimer(taskId);

        responseService.setCollectInitEvent(taskObject.getTaskId(), taskObject.getSessionId());
        ...
        if (programParam == null) {
            ...
            responseService.setCollectDoneEvent(taskObject.getTaskId(), taskObject.getSessionId(), -1);
        ...
        if (fileSystemService.templateMarkerExists(this.taskObject.getProgramId())) {
            ...
            String scriptName = managerParamRepository.getByParamName("COLLECT_NAME").getParamValue();
        ...
        responseService.setCollectDoneEvent(taskObject.getTaskId(), taskObject.getSessionId(), exitCode);
        if (exitCode != 0) {
            timerCreatorService.cancelTimer(timer);
            eventPublisher.publishTaskEvent(ERROR, taskObject);
            return CompletableFuture.completedFuture(ERROR);
