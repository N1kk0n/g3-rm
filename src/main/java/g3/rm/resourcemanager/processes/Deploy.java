...
public class Deploy {
    ...
    public CompletableFuture<String> start() {
        ...
        Timer timer = timerService.createDeployTimer(taskId);

        responseService.sendDeployInitResponse(taskObject.getTaskId(), taskObject.getSessionId());
        ...
        if (taskParam == null) {
            ...
            responseService.sendDeployDoneResponse(this.taskObject.getTaskId(), taskObject.getSessionId(),-1);
        ...
        if (fileSystemService.templateMarkerExists(this.taskObject.getProgramId())) {
            ...
            String scriptName = agentParamRepository.getByName("DEPLOY_NAME").getValue();
        ...
        responseService.sendDeployDoneResponse(this.taskObject.getTaskId(), taskObject.getSessionId(), exitCode);
        if (exitCode != 0) {
            timerService.cancelTimer(timer);
            eventPublisher.publishTaskEvent(ERROR, taskObject);
            return CompletableFuture.completedFuture(ERROR);
