...
public class CheckTaskTimer extends TimerTask {
    @Autowired
    ...
    private final Logger LOGGER = LogManager.getLogger("CheckTaskTimer");
    ...
    @Override
    public void run() {
        LOGGER.info("Interrupting check task procedure. Task ID: " + taskId);

        Iterable<TaskProcess> processes = processRepository.findAllByEntityIdAndOperation(taskId, "CHECK");
