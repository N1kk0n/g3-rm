...
public class DeployTimer extends TimerTask {
    @Autowired
    ...
    private final Logger LOGGER = LogManager.getLogger("DeployTimer");
    ...
    @Override
    public void run() {
        LOGGER.info("Interrupting deploy procedure. Task ID: " + taskId);

        Iterable<TaskProcess> processes = processRepository.findAllByEntityIdAndOperation(taskId, "DEPLOY");
